...
static inline swcProcessorType swcWhoAmI(void)
{
    ...
    #endif
    #ifdef AARCH64_CORE
        // We are one of the ARM core processors
        unsigned int coreId = 0xFF;
        asm ("mrs %[result], mpidr_el1" : [result] "=r" (coreId) : : );
        unsigned int processorIndex = coreId & 0xFF;
        volatile swcProcessorType swcProcessor;
        switch(processorIndex)
        {
            case 0: swcProcessor = PROCESS_ARM0; break;
            case 1: swcProcessor = PROCESS_ARM1; break;
            case 2: swcProcessor = PROCESS_ARM2; break;
            case 3: swcProcessor = PROCESS_ARM3; break;
            default: swcProcessor = ERROR_ID;
        }
        return swcProcessor;
    #endif
    #ifdef ARM_CORE
